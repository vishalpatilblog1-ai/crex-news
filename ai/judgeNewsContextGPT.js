// news/ai/judgeNewsContextGPT.js
// GPT fallback for judgeNewsContext.js — same input/output contract.
// Used when the Claude (Haiku) judge call fails, e.g. Claude API quota/outage.
...
export async function judgeNewsContextGPT({
  articleText,
  existingContexts = [],
}) {
...
`;
  ...
  if (typeof parsed.newContext !== "string" || parsed.newContext.length < 20) {
    throw new Error("Invalid newContext from GPT");
  }

  if (typeof parsed.isAlreadyCovered !== "boolean") {
    throw new Error("Invalid isAlreadyCovered flag from GPT");
  }

  if (parsed.matchedIndex !== null && typeof parsed.matchedIndex !== "number") {
    throw new Error("Invalid matchedIndex from GPT");
  }
  ...
  if (
    ...
    throw new Error("Invalid confidence score from GPT");
  }
