...
`;
  ...
  // RETRY (not trim) on overshoot: cutting the tweet at 280 risks chopping
  // off the closing verdict, which is usually the whole point of the tweet.
  // Only retries once, and only when it actually overshoots -- this is NOT
  // the old "retry on every call" pattern, it's a rare safety net for when
  // the strict 200-280 prompt instruction still gets missed.
  ...
  if (tweetText.length > 280 && isRetry) {
    // Retry also came back over -- give up and let it post rather than
    // burning a third call. A slightly-long complete tweet beats an
    // incomplete truncated one.
    console.warn(
