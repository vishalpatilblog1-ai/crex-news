...
// Default 200–280 applies to every source unless overridden below.
// CB (Cricbuzz) gets extra room ONLY when isLongTweetEligible() clears the
// article first (source is original + article actually has enough substance
// to justify more length) — this is a ceiling, not a target bump for every CB tweet.
...
- Target length: STRICT ${MIN_CHARS}–${MAX_CHARS} characters, no exceptions.
  If content would run longer, trim to its sharpest clause rather than exceeding ${MAX_CHARS}.
  Never go under ${MIN_CHARS} or over ${MAX_CHARS}.
...
`
    : `
─────────────────────────────────────────
PLAYER FIELD (required — output after tweet)
─────────────────────────────────────────
No card needed for this article type. Still output a JSON block on a new line,
IMMEDIATELY after the tweet with NO blank line between them, in this exact format:
CARD_JSON:{"player":"Rohit Sharma"}

- player: the primary player's full name this article centers on (e.g. "Rohit Sharma"),
  or "" if no single player is central to this article.
Do not add any explanation around the JSON.
`
}
`;
  ...
  try {
    ...
    if (markerIndex !== -1) {
      ...
      if (jsonMatch) {
        try {
          ...
          console.warn("⚠️ Failed to parse CARD_JSON:", jsonMatch[0]);
        }
      } else {
        console.warn(
    ...
    // console.log(`🏃 Player:`, player || "none");

    return { tweetText, card, player };
  } catch (err) {
    // console.error("❌ GPT Tweet Generation Error:", err);
    console.log("❌ GPT Tweet Generation Error:");
    return { tweetText: null, card: null, player: null };
  }
}
...
export async function generateGPTTweetWithType(
  articleText,
  articleType,
  source = null,
  isLongEligible = false,
  ...
  try {
    const { tweetText, card, player } = await _generateTweet(
      articleText,
      resolvedType,
      false,
      source,
    ...
    return { tweetText, articleType: resolvedType, card, player };
  } catch (err) {
    console.error("❌ GPT Tweet Generation Error:", err);
    return {
      tweetText: null,
